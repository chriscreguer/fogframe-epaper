...
# Neutral tones are dithered black/white, because the panel has no grey.
# Blue noise rather than an ordered Bayer grid: same average density, but no
# periodic structure, so flat fog reads as tone instead of regularly spaced
# dots competing with the exploration lines. Generated by make_dither.py;
# Bayer is kept as a fallback if the mask is missing.
_BAYER8 = np.array([
...
_BLUE_NOISE_PATH = Path(__file__).parent / "assets" / "bluenoise.npy"


def _dither_mask():
    if _BLUE_NOISE_PATH.exists():
        m = np.load(_BLUE_NOISE_PATH).astype(np.float32)
        return (m + 0.5) / m.size
    return _BAYER8
